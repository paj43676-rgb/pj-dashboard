...
function normalizeImpact(value = 'low') {
...
function inferNewsImpact(title = '', source = '') {
  const text = `${title} ${source}`.toLowerCase();
  if (/(warsh|powell|fed chair|jackson hole|cpi|pce|nfp|nonfarm|payroll|rate hike|rate cut|inflation|fomc|ecb|boj|boe|gdp|jobs report|retail sales|benchmark payroll)/i.test(text)) return 'high';
  if (/(price forecast|steady|holds|range|speech|minutes|yields|etf inflow|auction|claims|sentiment|consumer confidence|technical backdrop|bullish|bearish)/i.test(text)) return 'medium';
  return 'low';
}
  ...
  return cal
    .map((item) => {
      ...
      return {
        ...
        assetIds: countryToAssets(item.c || item.country),
